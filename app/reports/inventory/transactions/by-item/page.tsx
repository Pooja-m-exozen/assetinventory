...
// Group transactions by item (Inventory Tag ID)
const groupTransactionsByItem = (transactions: Transaction[]) => {
  const grouped: { [key: string]: Transaction[] } = {};
  transactions.forEach((transaction) => {
    if (!grouped[transaction.inventoryTagId]) {
      grouped[transaction.inventoryTagId] = [];
    }
    grouped[transaction.inventoryTagId].push(transaction);
  });
  return grouped;
};

export default function InventoryTransactionsByItemPage() {
  const [currentPage, setCurrentPage] = useState(1);
  const [itemsPerPage, setItemsPerPage] = useState(100);
  const [searchItem, setSearchItem] = useState("");

  const groupedTransactions = groupTransactionsByItem(dummyTransactions);
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                {Object.entries(groupedTransactions).map(([itemKey, transactions]) => (
                  <>
                    {/* Item Group Header */}
                    <tr key={`header-${itemKey}`} className="bg-gray-100 dark:bg-gray-700">
                      <td colSpan={6} className="px-6 py-2 text-sm font-medium text-gray-900 dark:text-gray-200" style={{ fontFamily: 'Inter, system-ui, -apple-system, sans-serif' }}>
                        Item : {itemKey}
                      </td>
                    </tr>
                    {/* Transaction Rows */}
