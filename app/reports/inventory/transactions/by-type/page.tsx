...
const groupTransactionsByType = (transactions: Transaction[]) => {
  ...
  transactions.forEach((transaction) => {
    ...
    grouped[transaction.transactionType].push(transaction);
  });
  return grouped;
};

export default function InventoryTransactionsByTypePage() {
  const [currentPage, setCurrentPage] = useState(1);
  const [itemsPerPage, setItemsPerPage] = useState(100);
  const [searchItem, setSearchItem] = useState("");

  const groupedTransactions = groupTransactionsByType(dummyTransactions);
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                {Object.entries(groupedTransactions).map(([typeKey, transactions]) => (
                  <>
                    {/* Transaction Type Group Header */}
                    <tr key={`header-${typeKey}`} className="bg-orange-50 dark:bg-orange-900/20">
