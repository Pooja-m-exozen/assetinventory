...
// Group transactions by event date
const groupTransactionsByDate = (transactions: Transaction[]) => {
  const grouped: { [key: string]: Transaction[] } = {};
  transactions.forEach((transaction) => {
    if (!grouped[transaction.eventDate]) {
      grouped[transaction.eventDate] = [];
    }
    grouped[transaction.eventDate].push(transaction);
  });
  return grouped;
};

export default function InventoryTransactionsByDatePage() {
  const [currentPage, setCurrentPage] = useState(1);
  const [itemsPerPage, setItemsPerPage] = useState(100);
  const [searchItem, setSearchItem] = useState("");

  const groupedTransactions = groupTransactionsByDate(dummyTransactions);
            ...
            <table className="w-full">
              ...
              <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                {Object.entries(groupedTransactions).map(([date, transactions]) => (
                  <>
                    {/* Date Group Header */}
                    <tr key={`header-${date}`} className="bg-yellow-50 dark:bg-yellow-900/20">
