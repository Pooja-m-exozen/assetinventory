...
const getWeekRange = (dateStr: string): { from: string; to: string } => {
...
const groupTransactionsByWeek = (transactions: Transaction[]) => {
  ...
  transactions.forEach((transaction) => {
    const weekRange = getWeekRange(transaction.eventDate);
...
export default function InventoryTransactionsByWeekPage() {
  const [currentPage, setCurrentPage] = useState(1);
  const [itemsPerPage, setItemsPerPage] = useState(100);
  const [searchItem, setSearchItem] = useState("");

  const groupedTransactions = groupTransactionsByWeek(dummyTransactions);
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                {Object.entries(groupedTransactions).map(([weekKey, transactions]) => {
                  const [fromDate, toDate] = weekKey.split("-");
                  return (
                    <>
                      {/* Week Group Header */}
                      <tr key={`header-${weekKey}`} className="bg-orange-50 dark:bg-orange-900/20">
                        <td colSpan={6} className="px-6 py-2 text-sm font-medium text-gray-900 dark:text-gray-200" style={{ fontFamily: 'Inter, system-ui, -apple-system, sans-serif' }}>
                          Week of Event Date : From {fromDate} To {toDate}
                        </td>
                      </tr>
                      {/* Transaction Rows */}
