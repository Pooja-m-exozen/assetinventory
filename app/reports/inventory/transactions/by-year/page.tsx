...
const getYearKey = (dateStr: string): string => {
...
const groupTransactionsByYear = (transactions: Transaction[]) => {
  ...
  transactions.forEach((transaction) => {
    const yearKey = getYearKey(transaction.eventDate);
    if (!grouped[yearKey]) {
      grouped[yearKey] = [];
    }
    grouped[yearKey].push(transaction);
  });
  return grouped;
};

export default function InventoryTransactionsByYearPage() {
  const [currentPage, setCurrentPage] = useState(1);
  const [itemsPerPage, setItemsPerPage] = useState(100);
  const [searchItem, setSearchItem] = useState("");

  const groupedTransactions = groupTransactionsByYear(dummyTransactions);
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                {Object.entries(groupedTransactions).map(([yearKey, transactions]) => (
                  <>
                    {/* Year Group Header */}
                    <tr key={`header-${yearKey}`} className="bg-orange-50 dark:bg-orange-900/20">
