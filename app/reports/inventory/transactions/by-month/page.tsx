...
const groupTransactionsByMonth = (transactions: Transaction[]) => {
  ...
  transactions.forEach((transaction) => {
    const monthKey = getMonthKey(transaction.eventDate);
    if (!grouped[monthKey]) {
      grouped[monthKey] = [];
    }
...
export default function InventoryTransactionsByMonthPage() {
  const [currentPage, setCurrentPage] = useState(1);
  const [itemsPerPage, setItemsPerPage] = useState(100);
  const [searchItem, setSearchItem] = useState("");

  const groupedTransactions = groupTransactionsByMonth(dummyTransactions);
            ...
            <table className="w-full">
              ...
              <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                {Object.entries(groupedTransactions).map(([monthKey, transactions]) => (
                  <>
                    {/* Month Group Header */}
                    <tr key={`header-${monthKey}`} className="bg-orange-50 dark:bg-orange-900/20">
