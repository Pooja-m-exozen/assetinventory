...
export const getCategories = async (
  ...
  const endpoint = `/v1/categories?${params}`;
  return apiRequest<PaginatedResponse<Category>>(endpoint);
};

// Get single category by ID
export const getCategory = async (id: number | string): Promise<{ data: Category }> => {
  const endpoint = `/v1/categories/${id}`;
  return apiRequest<{ data: Category }>(endpoint);
};

// Create category
export const createCategory = async (data: CreateCategoryData): Promise<{ data: Category }> => {
...
export const bulkDeleteCategories = async (
  ...
  try {
    const endpoint = `/v1/categories/bulk-delete`;
    ...
    if (err.message?.includes("bulk") || err.message?.includes("not found")) {
      ...
      if (deletedCount > 0) {
        return { message: `Deleted ${deletedCount} category(s)`, deletedCount };
      } else {
        throw new Error(errors.join(", ") || "Failed to delete categories");
      }
    }
    throw err;
  }
...
export const importCategories = async (
  ...
  failedCount: number;
  errors?: Array<{ row: number; category: string; error: string }>;
}> => {
  const token = getAuthToken();
  const formData = new FormData();
  formData.append("file", file);

  const response = await fetch(`${API_BASE_URL}/v1/categories/import`, {
    method: "POST",
    headers: {
      ...(token && { Authorization: `Bearer ${token}` }),
    },
...
export const exportCategories = async (
  ...
  let url = `${API_BASE_URL}/v1/categories/export?${params}`;
  ...
  if (!response.ok && response.status === 404) {
    try {
      ...
      if (
        ...
        url = `${API_BASE_URL}/v1/categories/export-data?${params}`;
